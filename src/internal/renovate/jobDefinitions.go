...
// otelEnvVarsForJobs returns OTel-related env vars (OTEL_* and TRACEPARENT) to
// forward to Renovate Jobs when RENOVATE_FORWARD_OTEL is set to "true". This
// enables Renovate's built-in OTLP export in Job containers. Returns nil when
// forwarding is disabled or no OTLP endpoint is resolved.
