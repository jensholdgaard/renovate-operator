...
func TestOtelEnvVarsForJobs(t *testing.T) {
	t.Run("injects TRACEPARENT when provided and forwarding enabled", func(t *testing.T) {
		t.Setenv("RENOVATE_FORWARD_OTEL", "true")
		t.Setenv("OTEL_EXPORTER_OTLP_ENDPOINT", "http://collector:4318")

		envs := otelEnvVarsForJobs("00-abc123-def456-01")

		var found bool
		for _, env := range envs {
			if env.Name == "TRACEPARENT" {
				found = true
				if env.Value != "00-abc123-def456-01" {
					t.Fatalf("expected TRACEPARENT=00-abc123-def456-01, got %s", env.Value)
				}
			}
		...
		if !found {
			t.Fatal("expected TRACEPARENT env var to be present")
		}
	})

	t.Run("omits TRACEPARENT when empty", func(t *testing.T) {
		t.Setenv("RENOVATE_FORWARD_OTEL", "true")
		t.Setenv("OTEL_EXPORTER_OTLP_ENDPOINT", "http://collector:4318")
		...
		for _, env := range envs {
			if env.Name == "TRACEPARENT" {
				t.Fatal("expected no TRACEPARENT env var when traceparent is empty")
			}
		}
	})
...
func TestOtelWrapperInjection(t *testing.T) {
	_ = config.InitializeConfigModule([]config.ConfigItemDescription{
		{Key: "JOB_TIMEOUT_SECONDS", Optional: true, Default: "10"},
	})
	...
	job := &api.RenovateJob{
		ObjectMeta: metav1.ObjectMeta{Name: "rj", Namespace: "ns"},
		Spec:       api.RenovateJobSpec{Image: "renovate:latest"},
	}

	t.Run("executor uses wrapper when otel forwarding enabled with traceparent", func(t *testing.T) {
		t.Setenv("RENOVATE_FORWARD_OTEL", "true")

		rj := newRenovateJob(job, "myorg/myrepo", "00-aaa-bbb-01")
		c := expectContainer(t, rj)

		if c.Command[0] != "/bin/sh" {
			t.Fatalf("expected command /bin/sh, got %v", c.Command)
		}
		if len(c.Args) != 1 || !strings.Contains(c.Args[0], "otel-wrapper.js") {
			t.Fatalf("expected args to contain otel-wrapper.js, got %v", c.Args)
		}
		if !strings.Contains(c.Args[0], "myorg/myrepo") {
			t.Fatalf("expected args to contain project name, got %v", c.Args)
		}
	})

	t.Run("executor uses direct command without traceparent", func(t *testing.T) {
		t.Setenv("RENOVATE_FORWARD_OTEL", "true")

		rj := newRenovateJob(job, "myorg/myrepo", "")
		c := expectContainer(t, rj)

		if c.Command[0] != "renovate" {
			t.Fatalf("expected direct renovate command, got %v", c.Command)
		}
	})

	t.Run("discovery uses wrapper when otel forwarding enabled with traceparent", func(t *testing.T) {
		t.Setenv("RENOVATE_FORWARD_OTEL", "true")

		dj := newDiscoveryJob(job, "00-aaa-bbb-01")
		c := expectContainer(t, dj)

		if !strings.Contains(c.Args[0], "otel-wrapper.js") {
			t.Fatalf("expected args to contain otel-wrapper.js, got %v", c.Args)
		}
		if !strings.Contains(c.Args[0], "--autodiscover") {
			t.Fatalf("expected args to contain --autodiscover, got %v", c.Args)
		}
	})
}
